...
export class DefaultDatePipe implements PipeTransform {
  ...
  private readonly formatConfig = ObjectUtilsService.clone(datePipeTimeFormatConfig);
  ...
  transform(
    ...
    const dateFormatWithOrWithoutSeconds = showSeconds ? format : format.replace(':ss', '');

    try {
      return this.datePipe.transform(date, dateFormatWithOrWithoutSeconds, timezone) || '';
    } catch (error) {
